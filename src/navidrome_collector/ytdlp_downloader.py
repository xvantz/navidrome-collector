...
def search_and_download(query: str, output_dir: str | Path, max_duration: int = 600) -> Optional[Path]:
    ...
    Returns:
        Path to the downloaded (and tagged) file, or None if failed.
    """
    ...
    # Parse output: lines from --print
    lines = [l.strip() for l in result.stdout.split("\n") if l.strip()]

    metadata = _parse_metadata(lines)
    file_path = None

    # First line should be the filepath (after_move:filepath)
    ...
    # Tag the file with available YouTube metadata
    if metadata:
...
def _parse_metadata(lines: list[str]) -> Optional[dict]:
    ...
    Output format (5 lines per video):
      1. filepath
      2. title
      3. channel
      4. uploader
...
def _tag_file(path: Path, meta: dict) -> None:
    ...
    # Try to parse "Artist - Title" from YouTube title
    artist = channel
    title = yt_title

    # Many YouTube titles are "Artist - Song (Official Video)"
    import re
